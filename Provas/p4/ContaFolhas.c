#include<stdio.h>
#include<stdlib.h>
#include<string.h>

struct No
{
    int chave;
    struct No *pai;
    struct No *direita;
    struct No *esquerda;
};

struct Contador
{
    int qt;
};

struct Arvore
{
    struct No *raiz;
};


struct Arvore * criaArvore();
struct No * criaNo(int);
void inserir(struct Arvore *, struct No *);
struct Contador * contaNos(struct No *, struct Contador *);


int main()
{
    int chave;
    int n, i;
    struct Contador *folhas=(struct Contador*)malloc(sizeof(struct Contador*)), *qtfinal=(struct Contador*)malloc(sizeof(struct Contador*));
    struct Arvore *arvore = criaArvore();

    folhas->qt = 0;
    qtfinal->qt = 0;

    scanf("%d", &n);

    for(i=0;i<n;i++)
    {
        scanf("%d", &chave);
        inserir(arvore,criaNo(chave));
    }

    qtfinal = contaNos(arvore->raiz, folhas);

    printf("%d\n", qtfinal->qt);

    return 0;
}

struct Arvore * criaArvore()
{
    struct Arvore *t = (struct Arvore*)malloc(sizeof(struct Arvore));
    if(t!=NULL)
    {
        t->raiz=NULL;
    }
    return t;
}

struct No * criaNo(int chave)
{
    struct No *no = (struct No *)malloc(sizeof(struct No));
    if(no!=NULL)
    {
        no->chave = chave;
        no->esquerda = NULL;
        no->direita = NULL;
        no->pai = NULL;
    }
    return no;
}

void inserir(struct Arvore *t, struct No *novo)
{
    struct No *pai=NULL, *filho=t->raiz;

    while(filho!=NULL)
    {

        pai = filho;

        if(filho->chave == novo->chave)
        {
            return;
        }

        if(filho->chave < novo->chave)
        {
            filho = filho->direita;
        }

        else
        {
            filho = filho->esquerda;
        }
    }

    if(pai!=NULL)
    {
        novo->pai = pai;
        if(pai->chave > novo->chave)
        {
            pai->esquerda = novo;
        }
        else
        {
            pai->direita = novo;
        }
    }
    else
    {
        t->raiz = novo;
    }
}


struct Contador * contaNos(struct No *r, struct Contador *f)
{
    if(r!=NULL)
    {
        contaNos(r->esquerda, f);
        contaNos(r->direita, f);
    }

    if(r!=NULL && r->direita == NULL && r->esquerda == NULL)
    {
        f->qt++;
    }

    return f;
}


